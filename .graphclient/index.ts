...
import type { ZoranftsSubgraphContext } from './sources/zoranfts-subgraph/types';
...
export type UserResolvers<ContextType = MeshContext, ParentType extends ResolversParentTypes['User'] = ResolversParentTypes['User']> = ResolversObject<{
  id?: Resolver<ResolversTypes['ID'], ParentType, ContextType>;
  tokens?: Resolver<Array<ResolversTypes['Token']>, ParentType, ContextType, RequireFields<UsertokensArgs, 'skip' | 'first'>>;
  __isTypeOf?: IsTypeOfResolverFn<ParentType, ContextType>;
}>;
...
export type MeshContext = ZoranftsSubgraphContext & BaseMeshContext;
...
const importFn = (moduleId: string) => {
  ...
  switch(relativeModuleId) {
    case ".graphclient/sources/zoranfts-subgraph/introspectionSchema":
      return import("./sources/zoranfts-subgraph/introspectionSchema");
...
const zoranftsSubgraphTransforms = [];
const additionalTypeDefs = [] as any[];
const zoranftsSubgraphHandler = new GraphqlHandler({
              name: "zoranfts-subgraph",
              config: {"endpoint":"https://api.thegraph.com/subgraphs/name/sujiehua/zoranfts-subgraph"},
              baseDir,
              cache,
              pubsub,
              store: sourcesStore.child("zoranfts-subgraph"),
              logger: logger.child("zoranfts-subgraph"),
              importFn,
            });
sources[0] = {
          name: 'zoranfts-subgraph',
          handler: zoranftsSubgraphHandler,
          transforms: zoranftsSubgraphTransforms
        }
...
export type ExampleQueryQuery = { users: Array<(
    Pick<User, 'id'>
    & { tokens: Array<Pick<Token, 'id' | 'ownerId' | 'tokenId' | 'image' | 'metadata' | 'kind' | 'seed'>> }
  )> };
